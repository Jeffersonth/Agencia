    ...
    <>
      ...
      <Section labelledBy="dores">
        ...
        <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {sector.pains.map((p) => (
            <div key={p.title} className="rounded-[var(--radius-card)] border border-line bg-mist p-6">
          ...
          {sector.solutions.map(({ service: slug, why }) => {
            ...
            return (
              <Link
                key={slug}
                href={serviceHref(s)}
                className="group flex gap-5 rounded-[var(--radius-card)] border border-line bg-white p-7 shadow-[var(--shadow-card)] transition-all hover:-translate-y-0.5 hover:border-signal/40 hover:shadow-[var(--shadow-lift)]"
              >
